...
// do we need to filter out redundant results? use set
// do we need to restrict maximum number of results? can't search forever...
...
const find_images_at = async (query, cursor) => {
  ...
  //const urls = parse_image_urls(json.items_html)
...
const profile_image = async username => {
  ...
  const profile_image_url = $("img.ProfileAvatar-image").attr('src')
  console.log(`downloading profile image for ${username} @ ${profile_image_url}`)
  const image_res = await fetch(profile_image_url)
  const profile_image = await image_res.buffer()
